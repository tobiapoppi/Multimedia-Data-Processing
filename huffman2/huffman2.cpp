...
struct huffman {
	...
	struct compare_symbols {
		bool operator()(const std::pair<uint8_t, huffman::node*>& lhs, const std::pair<uint8_t, huffman::node*>& rhs) {
			if (lhs.second->len_ != rhs.second->len_) {
				return lhs.second->len_ < rhs.second->len_;
			}
			else {
	...
	void calculate_lengths(node* n, uint8_t len = 0) {
		if (n->left_ == nullptr) {
			n->len_ = len;
			symbols_[n->sym_] = n;
		}
		else {
			calculate_lengths(n->left_, len + 1);
			calculate_lengths(n->right_, len + 1);
		}
	}
	
	std::unordered_map<uint8_t, node*> gen_canonical_codes(std::unordered_map<uint8_t, node*> s) {
		std::vector<std::pair<uint8_t, node*>> v(s.begin(), s.end());
		std::sort(v.begin(), v.end(), compare_symbols());
		uint64_t val = 0;
		uint64_t len = 0;
		for (auto& x : v) {
			x.second->code_ = val;
			while (x.second->len_ != len) {
				++len;
				x.second->code_ <<= 1;
				val <<= 1;
			}
		...
		std::unordered_map<uint8_t, node*> map(v.begin(), v.end());
		return map;
	}
	...
	huffman(_InIt _First, _InIt _Last) {
		...
		calculate_lengths(root);
		symbols_ = gen_canonical_codes(symbols_);
	}
...
void gen_canonical_for_triplets(std::vector<triplet>& s) {
...
void decode(std::istream& is, std::ostream& os) {
	...
	//generate canonical codes for triplets
	gen_canonical_for_triplets(vh);
...
int main(int argc, char* argv[]) {
	...
	if (argc != 4) {
		error("SYNTAX: huffman2 [c|d] <input file> <output file>\n");
	}
